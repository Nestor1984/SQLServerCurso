...
-- Ej 1: Vamos a ver cuantos pedidos tiene cada cliente en mi base de datos
/* Seleccionamos de nuestra tabla ordenes el campo id_orden y de nuestra tabla clientes el campo nombre de nuestra tabla ordenes, 
y ahora hago un inner join para hacer un enlace interno a mi tabla clientes, on es decir en donde el campo idcliente de mi tabla ordenes
sea igual a mi campo idcliente de mi tabla clientes*/ 

select ordenes.id_orden, clientes.nombre from ordenes
inner join clientes on ordenes.idcliente = clientes.idcliente;

-- Ej 2
select ordenes.id_orden, clientes.nombre, clientes.apellido from ordenes
inner join clientes on ordenes.idcliente = clientes.idcliente;

-- Ej 3: Vamos traer en que fecha cada cliente genero esa orden
select ordenes.id_orden, clientes.nombre, clientes.apellido, ordenes.fecha_orden from ordenes
inner join clientes on ordenes.idcliente = clientes.idcliente;
...
-- Ej6 : quiero saber que vendedores realizaron esta facturacion en mi sistema
select ordenes.id_orden, clientes.nombre, clientes.apellido, ordenes.fecha_orden, ordenes.id_vendedor from ordenes
inner join clientes on ordenes.idcliente = clientes.idcliente
where nombre = 'Juan'
order by fecha_orden;
