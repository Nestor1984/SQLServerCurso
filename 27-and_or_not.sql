...
ciudad varchar(30) not null
  ...
  -- Quiero ver los clientes de Italia, que sean unicamente los que viven en Roma.
  select * from clientes
  where pais = 'Italia' and ciudad = 'Roma';

  --  Quiero ver los clientes que tengo en Roma o Benecia
  select * from clientes
  where ciudad = 'Roma' or ciudad = 'Venecia';
  
  -- NOTA: NO PUEDO USAR DOBLE O MAS AND, PORQUE PARA ESO ESTA EL OPERADOR IN.
  select * from clientes
  where pais = 'Italia' and ciudad in('Roma', 'Venecia');

  -- Quiero consultar los valores de mis clientes que no sean de Alemania
   select * from clientes
